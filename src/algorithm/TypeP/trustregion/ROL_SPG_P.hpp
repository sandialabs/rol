...
public:
  ...
  SPG_P(ParameterList &parlist) {
   ...
   maxitInc_  = list.sublist("Solver").get("Iteration Increase Limit",      2);
   ...
   // Subsolver (spectral proximal gradient) parameters
  ...
  Real alpha(1), lambda(1), lambdaTmp(1);
  ...
  //pwa->set(y); pwa->axpy(-one,x);// compute s 
  ss        = pwa->dot(*pwa); 
  ...
  mnew  = mold;  
  if (verbosity_ > 1)
    outStream << "  Spectral Proximal Gradient"          << std::endl;

  SPiter_ = 0;
  while (SPiter_ < maxit_) {
    // Compute initial spectral step size
    if (sHs <= 0) {
      lambdaTmp = one / gmod.norm();
    }
    else {
      lambdaTmp = ss / sHs;
    }
    ...
    dprox(*pwa,x,lambda,del,nobj,nprox_,outStream); // pwa  = P(y-lambda gmod.dual())
    ...
    sHs   = pwa->apply(*dwa);                 // sHs = <step, H step>
    gs    = pwa->apply(gmod.dual());          // gs  = <step, model gradient>
    ...
    // Compute proximal gradient norm
    pwa1->set(y); pwa1->axpy(-t0_, gmod.dual()); // again, pwa = y - t g
    dprox(*pwa1, x,t0_,del,nobj,nprox_,outStream); // stashes in pwa
    pwa1->axpy(-one,y);
    gnorm = pwa1->norm() / t0_;
    if (verbosity_ > 1) {
      outStream << std::endl;
    ...
    if (gnorm < std::max(gtol, std::sqrt(tol))) break;
  }
  if (useNMSP_ && useMin_) {
...
void dprox(Vector<Real>       &x,
  ...
  Ptr<Vector<Real>> py0, py1, pyc, px;
  py0 = pwa2->clone(); 
  py1 = pwa3->clone(); 
  pyc = pwa4->clone(); 
  px = pwa5->clone(); 
  ...
  int cnt(nprox_++);
  nobj.prox(*py1, x, t, tol); nprox_++;
  px->set(*py1); px->axpy(-one,x0);
  f1 = px->norm();
  if (f1 <= del) {
    x.set(*py1);
    return;
  ...
  while (true) {
    if (std::abs(fc-del) < std::abs(f1-del)) {
      t0 = t1; t1 = tc; tc = t0;
      f0 = f1; f1 = fc; fc = f0;
      py0->set(*py1); py1->set(*pyc); pyc->set(*py0);
    }
