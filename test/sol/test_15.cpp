...
int main(int argc, char* argv[]) {
  ...
  try {
    ...
    std::string filename = "input_15.xml";
...
//    ROL::ParameterList list = *parlist;
    ...
    *outStream << "Check Derivatives of Parametrized Objective Function\n";
    ...
    ROL::PrimalDualRisk<RealT> pd_solver(problem,sampler,*parlist);
    pd_solver.check(*outStream);
    pd_solver.run(*outStream);
    ...
    // Print solutions
    *outStream << std::endl << "Primal Dual Risk Solution" << std::endl;
