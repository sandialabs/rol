...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    *outStream << "  " << ocType << "-optimal design time:      "
    ...
    dname << ocType << "_optimal_design_ex09";
    factory->printDesign(dname.str());

    // Run greedy algorithm
    auto cost = factory->createDesignVector();
    cost->setScalar(static_cast<RealT>(1));
    RealT budget = parlist->sublist("Problem").get("Budget",4.0);
    ...
    auto CoptVec = theta->clone();
    CoptVec->setScalar(1.0);
    if (ocType == "A") {
      parlist->sublist("OED").sublist("Greedy").set("Use Deletion",true);
      std::clock_t timerD = std::clock();
      auto pad = ROL::OED::GreedyAlgorithmA<RealT>(model,obs,sampler,theta,M,cost,budget,*parlist,*outStream);
      *outStream << "  A-optimal greedy deletion design time:      "
                 << static_cast<RealT>(std::clock()-timerD)/static_cast<RealT>(CLOCKS_PER_SEC)
                 << " seconds" << std::endl;
      ROL::OED::PrintDesign<RealT>(pad,sampler,"GreedyDeletionDesignA_ex09",".txt");
      parlist->sublist("OED").sublist("Greedy").set("Use Deletion",false);
      std::clock_t timerA = std::clock();
      auto paa = ROL::OED::GreedyAlgorithmA<RealT>(model,obs,sampler,theta,M,cost,budget,*parlist,*outStream);
      *outStream << "  A-optimal greedy addition design time:      "
                 << static_cast<RealT>(std::clock()-timerA)/static_cast<RealT>(CLOCKS_PER_SEC)
                 << " seconds" << std::endl;
      ROL::OED::PrintDesign<RealT>(paa,sampler,"GreedyAdditionDesignA_ex09",".txt");
    }
    else if (ocType == "C") {
      parlist->sublist("OED").sublist("Greedy").set("Use Deletion",true);
      auto pcd = ROL::OED::GreedyAlgorithmC<RealT>(model,obs,sampler,theta,M,CoptVec,cost,budget,*parlist,*outStream);
      std::clock_t timerD = std::clock();
      *outStream << "  C-optimal greedy deletion design time:      "
                 << static_cast<RealT>(std::clock()-timerD)/static_cast<RealT>(CLOCKS_PER_SEC)
                 << " seconds" << std::endl;
      ROL::OED::PrintDesign<RealT>(pcd,sampler,"GreedyDeletionDesignC_ex09",".txt");
      parlist->sublist("OED").sublist("Greedy").set("Use Deletion",false);
      std::clock_t timerA = std::clock();
      auto pca = ROL::OED::GreedyAlgorithmC<RealT>(model,obs,sampler,theta,M,CoptVec,cost,budget,*parlist,*outStream);
      *outStream << "  C-optimal greedy addition design time:      "
                 << static_cast<RealT>(std::clock()-timerA)/static_cast<RealT>(CLOCKS_PER_SEC)
                 << " seconds" << std::endl;
      ROL::OED::PrintDesign<RealT>(pca,sampler,"GreedyAdditionDesignC_ex09",".txt");
    }
    else if (ocType == "D") {
      parlist->sublist("OED").sublist("Greedy").set("Use Deletion",true);
      std::clock_t timerD = std::clock();
      auto pdd = ROL::OED::GreedyAlgorithmD<RealT>(model,obs,sampler,theta,M,cost,budget,*parlist,*outStream);
      *outStream << "  D-optimal greedy deletion design time:      "
                 << static_cast<RealT>(std::clock()-timerD)/static_cast<RealT>(CLOCKS_PER_SEC)
                 << " seconds" << std::endl;
      ROL::OED::PrintDesign<RealT>(pdd,sampler,"GreedyDeletionDesignD_ex09",".txt");
      parlist->sublist("OED").sublist("Greedy").set("Use Deletion",false);
      std::clock_t timerA = std::clock();
      auto pda = ROL::OED::GreedyAlgorithmD<RealT>(model,obs,sampler,theta,M,cost,budget,*parlist,*outStream);
      *outStream << "  D-optimal greedy addition design time:      "
                 << static_cast<RealT>(std::clock()-timerA)/static_cast<RealT>(CLOCKS_PER_SEC)
                 << " seconds" << std::endl;
      ROL::OED::PrintDesign<RealT>(pda,sampler,"GreedyAdditionDesignD_ex09",".txt");
    }
    else if (ocType == "I") {
      parlist->sublist("OED").sublist("Greedy").set("Use Deletion",true);
      std::clock_t timerD = std::clock();
      auto pid = ROL::OED::GreedyAlgorithmI<RealT>(model,obs,sampler,theta,M,cost,budget,*parlist,*outStream);
      *outStream << "  I-optimal greedy deletion design time:      "
                 << static_cast<RealT>(std::clock()-timerD)/static_cast<RealT>(CLOCKS_PER_SEC)
                 << " seconds" << std::endl;
      ROL::OED::PrintDesign<RealT>(pid,sampler,"GreedyDeletionDesignI_ex09",".txt");
      parlist->sublist("OED").sublist("Greedy").set("Use Deletion",false);
      std::clock_t timerA = std::clock();
      auto pia = ROL::OED::GreedyAlgorithmI<RealT>(model,obs,sampler,theta,M,cost,budget,*parlist,*outStream);
      *outStream << "  I-optimal greedy addition design time:      "
                 << static_cast<RealT>(std::clock()-timerA)/static_cast<RealT>(CLOCKS_PER_SEC)
                 << " seconds" << std::endl;
      ROL::OED::PrintDesign<RealT>(pia,sampler,"GreedyAdditionDesignI_ex09",".txt");
    }
    model->printState("state_ex09.txt",*theta->getVector(),nx);
  }
