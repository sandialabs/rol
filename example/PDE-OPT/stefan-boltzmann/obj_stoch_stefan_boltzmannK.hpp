...
#ifndef PDEOPT_QOI_STOCHASTIC_STEFAN_BOLTZMANNK_HPP
#define PDEOPT_QOI_STOCHASTIC_STEFAN_BOLTZMANNK_HPP

#include "../TOOLS/qoiK.hpp"
#include "pde_stoch_stefan_boltzmannK.hpp"

template <class Real, class DeviceType>
class QoI_StateCost : public QoI<Real, DeviceType> {
public:
...
private:
  ...
  Real cost(Real u, int deriv = 0) const {
    if ( exp_ ) {
      if ( deriv == 1 ) {
...
public:
  QoI_StateCost(const ROL::Ptr<fe_type> &fe,
  ...
  void gradient_1(scalar_view & grad,
    ...
    scalar_view WcostU("WcostU", c, p);
    fst::scalarMultiplyDataData(WcostU,weight_,costU);
    // Compute gradient of squared L2-norm of diff
    fst::integrate(grad,WcostU,fe_->NdetJ(),false);
  }
  ...
  void HessVec_11(scalar_view & hess,
    ...
    scalar_view WcostUV("WcostUV", c, p);
    fst::scalarMultiplyDataData(WcostUV,WcostU,valV_eval);
    // Compute gradient of squared L2-norm of diff
    fst::integrate(hess,WcostUV,fe_->NdetJ(),false);
  }
...
template <class Real, class DeviceType>
class QoI_ControlCost : public QoI<Real,DeviceType> {
public:
  using scalar_view = Kokkos::DynRankView<Real, DeviceType>;
  using basis_ptr   = Intrepid2::BasisPtr<DeviceType, Real, Real>;
  using fe_type     = FE<Real,DeviceType>;
...
private:
  ...
  const std::vector<ROL::Ptr<fe_type>> fe_bdry_;
  const std::vector<std::vector<int>> bdryCellLocIds_;
  std::vector<scalar_view> weight_;
  Real T_;
  bool exp_;
  Real rate1_;
  Real rate2_;
  ...
  Real cost(Real z, int deriv = 0) const {
    if ( exp_ ) {
      if ( deriv == 1 ) {
...
public:
  QoI_ControlCost(const ROL::Ptr<fe_type> &fe_vol,
                  const std::vector<ROL::Ptr<fe_type>> &fe_bdry,
    ...
    for (int l = 0; l < numLocSides; ++l) {
      ...
      if ( numCellsSide ) {
        const int numCubPerSide = fe_bdry_[l]->cubPts().extent_int(1);
        weight_[l] = scalar_view("weight", numCellsSide, numCubPerSide);
        for (int i = 0; i < numCellsSide; ++i) {
    ...
    if ( z_coeff != scalar_view() ) {
      ...
      for (int l = 0; l < numLocSides; ++l) {
        ...
        if ( numCellsSide ) {
          ...
          // Multiply cost and weight
          scalar_view WcostZ("WcostZ", numCellsSide, numCubPerSide);
          fst::scalarMultiplyDataData(WcostZ,weight_[l],costZ);
          // Compute gradient of squared L2-norm of diff
          scalar_view intGrad("intGrad",numCellsSide, f);
          fst::integrate(intGrad,WcostZ,fe_bdry_[l]->NdetJ(),false);
          // Add to integral value
          for (int i = 0; i < numCellsSide; ++i) {
            int cidx = bdryCellLocIds_[l][i];
            for (int j = 0; j < f; ++j) {
    ...
    if ( z_coeff != scalar_view() ) {
      ...
      for (int l = 0; l < numLocSides; ++l) {
        ...
        if ( numCellsSide ) {
          ...
          fe_bdry_[l]->evaluateValue(valZ_eval, z_coeff_bdry);
          // Evaluate direction on FE basis
          scalar_view v_coeff_bdry = getBoundaryCoeff(v_coeff, l);
          ...
          fst::scalarMultiplyDataData(WcostZ,weight_[l],costZ);
          // Multiply weighted cost and direction
          scalar_view WcostZV("WcostZV", numCellsSide, numCubPerSide);
          fst::scalarMultiplyDataData(WcostZV,WcostZ,valV_eval);
          // Compute hessian times a vector of cost
          scalar_view intHess("intHess", numCellsSide, f);
          fst::integrate(intHess,WcostZV,fe_bdry_[l]->NdetJ(),false);
          // Add to integral value
          for (int i = 0; i < numCellsSide; ++i) {
            int cidx = bdryCellLocIds_[l][i];
            for (int j = 0; j < f; ++j) {
...
template <class Real, class DeviceType>
class QoI_AdvectionCost : public QoI<Real,DeviceType> {
public:
  using scalar_view = Kokkos::DynRankView<Real, DeviceType>;
  using basis_ptr   = Intrepid2::BasisPtr<DeviceType, Real, Real>;
  using fe_type     = FE<Real,DeviceType>;
...
public:
  StochasticStefanBoltzmannStdObjective(ROL::ParameterList &parlist) {
...
public:
  StochasticStefanBoltzmannStdObjective3(ROL::ParameterList &parlist) {
    alpha0_ = parlist.sublist("Problem").get("State Cost",1.0);
    alpha1_ = parlist.sublist("Problem").get("Control Cost",1.0);
