  ...
  void computeDirichlet(void) {
    ...
    for (int i=0; i<numSidesets; ++i) {
      ...
      for (int j=0; j<numLocSides; ++j) {
        ...
        if (c > 0) feVel_->computeDofCoords(coords, bdryCellNodes_[i][j]);
        for (int k=0; k<c; ++k) {
          for (int l=0; l<f; ++l) {
...
public:
  PDE_NavierStokes(ROL::ParameterList &parlist) {
    ...
    for (int i=0; i<numFieldsCtrl_; ++i) {
      if (i==0) offsetCtrl_[i]  = 0;
      else      offsetCtrl_[i]  = offsetCtrl_[i-1] + basisPtrsCtrl_[i-1]->getCardinality();
  ...
  void Jacobian_1(scalar_view & jac,
    ...
    scalar_view valVel0("valVel0", c, p, d);
    scalar_view valVelDotgradPhi("valVelDotgralPhi", c, fv, p);
    computeViscosity(nu);
    feCtrl_->evaluateValue(valCtrl, Z[0]);
    imp_->compute(alpha, valCtrl, feVel_->cubPts(), 0);
    for (int i = 0; i < d; ++i) {
