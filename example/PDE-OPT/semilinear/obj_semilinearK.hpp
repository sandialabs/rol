...
#ifndef PDEOPT_QOI_L2TRACKING_SEMILINEARK_HPP
#define PDEOPT_QOI_L2TRACKING_SEMILINEARK_HPP

#include "../TOOLS/qoiK.hpp"
#include "pde_semilinearK.hpp"

template <class Real, class DeviceType>
class QoI_L2Tracking_Semilinear : public QoI<Real, DeviceType> {
public:
...
public:
  QoI_L2Tracking_Semilinear(const ROL::Ptr<fe_type> & fe) : fe_(fe) {
  ...
  void gradient_2(scalar_view & grad,
                  ...
                  const scalar_view z_coeff = scalar_view(),
                  const ROL::Ptr<const std::vector<Real> > & z_param = ROL::nullPtr) {
    throw Exception::Zero(">>> QoI_L2Tracking_Semilinear::gradient_2 is zero.");
  }
  ...
  void HessVec_12(scalar_view & hess,
                  ...
                  const scalar_view z_coeff = scalar_view(),
                  const ROL::Ptr<const std::vector<Real> > & z_param = ROL::nullPtr) {
    throw Exception::Zero(">>> QoI_L2Tracking_Semilinear::HessVec_12 is zero.");
  }
  ...
  void HessVec_21(scalar_view & hess,
                  ...
                  const scalar_view z_coeff = scalar_view(),
                  const ROL::Ptr<const std::vector<Real> > & z_param = ROL::nullPtr) {
    throw Exception::Zero(">>> QoI_L2Tracking_Semilinear::HessVec_21 is zero.");
  }
  ...
  void HessVec_22(scalar_view & hess,
                  ...
                  const scalar_view z_coeff = scalar_view(),
                  const ROL::Ptr<const std::vector<Real> > & z_param = ROL::nullPtr) {
    throw Exception::Zero(">>> QoI_L2Tracking_Semilinear::HessVec_22 is zero.");
  }

}; // QoI_L2Tracking

template <class Real, class DeviceType>
class QoI_L2Penalty_Semilinear : public QoI<Real, DeviceType> {
public:
  using scalar_view = Kokkos::DynRankView<Real, DeviceType>;
  using fe_type = FE<Real, DeviceType>;
  using fst = Intrepid2::FunctionSpaceTools<DeviceType>;
...
public:
  QoI_L2Penalty_Semilinear(const ROL::Ptr<fe_type> & fe) : fe_(fe) {}
  ...
  void gradient_1(scalar_view & grad,
                  ...
                  const scalar_view z_coeff = scalar_view(),
                  const ROL::Ptr<const std::vector<Real> > & z_param = ROL::nullPtr) {
    throw Exception::Zero(">>> QoI_L2Penalty_Semilinear::gradient_1 is zero.");
  }
  ...
  void HessVec_11(scalar_view & hess,
                  ...
                  const scalar_view z_coeff = scalar_view(),
                  const ROL::Ptr<const std::vector<Real> > & z_param = ROL::nullPtr) {
    throw Exception::Zero(">>> QoI_L2Penalty_Semilinear::HessVec_11 is zero.");
  }
  ...
  void HessVec_12(scalar_view & hess,
                  ...
                  const scalar_view z_coeff = scalar_view(),
                  const ROL::Ptr<const std::vector<Real> > & z_param = ROL::nullPtr) {
    throw Exception::Zero(">>> QoI_L2Penalty_Semilinear::HessVec_12 is zero.");
  }
  ...
  void HessVec_21(scalar_view & hess,
                  ...
                  const scalar_view z_coeff = scalar_view(),
                  const ROL::Ptr<const std::vector<Real> > & z_param = ROL::nullPtr) {
    throw Exception::Zero(">>> QoI_L2Penalty_Semilinear::HessVec_21 is zero.");
  }
...
template <class Real>
class StdObjective_Semilinear : public ROL::StdObjective<Real> {
private:
  Real alpha_;
